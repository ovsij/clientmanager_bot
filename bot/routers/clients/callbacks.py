...
from bot.keyboards.inline import inline_kb_create_complaint, inline_kb_create_invoice
...
async def callback_query_handler(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if code == 'createinvoice':
        ...
        text, reply_markup = inline_kb_create_invoice(next_text='Пришлите описание груза')
    ...
    if code.startswith('payment'):
        await state.update_data(payment=code.split('_')[1])
        data = await state.get_data()
        text, _ = inline_kb_create_invoice(next_text='Накладная готова, сейчас пришлем pdf', data=data)
    ...
    if code == 'createcomplaint':
        ...
        await state.update_data(not_finished=True)
        text, reply_markup = inline_kb_create_complaint(next_text='Пришлите номер накладной')
        msg = await callback_query.message.edit_text(text=text, reply_markup=reply_markup)
        await state.update_data(prev_msg=msg)
    ...
    if code == 'invioceid_continue':
        ...
        data = await state.get_data()
        text, reply_markup = inline_kb_create_complaint(next_text='Пришлите E-mail для ответа на претензию', data=data)
        await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
    if code == 'photo_continue':
        data = await state.get_data()
        
        text, _ = inline_kb_create_complaint(next_text='Преензия оформлена и направлена вашему менеджеру. Сейчас пришлем pdf.', data=data)
        await data['prev_msg'].edit_text(text=text)
