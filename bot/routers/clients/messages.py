...
from bot.keyboards.inline import inline_kb_create_complaint, inline_kb_create_invoice, inline_kb_payment_method
...
async def get_description(message: Message, state: FSMContext):
    ...
    text, reply_markup = inline_kb_create_invoice(next_text='Пришлите вес груза в килограммах', data=data)
...
async def get_weight(message: Message, state: FSMContext):
    ...
    try:
        ...
        data = await state.get_data()
        text, reply_markup = inline_kb_create_invoice(next_text='Пришлите габариты груза', data=data)
        await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
    except:
        data = await state.get_data()
        text, reply_markup = inline_kb_create_invoice(next_text='Пришлите сообщение с числом, равным весу груза в килограммах, без указания единицы измерения и других символов.', data=data)
        await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
...
async def get_dimensions(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    text, reply_markup = inline_kb_create_invoice(next_text='Пришлите точный адрес отправки груза', data=data)
    await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
...
async def get_pickup_address(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    text, reply_markup = inline_kb_create_invoice(next_text='Пришлите точный адрес получения груза', data=data)
    await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
...
async def get_invoice_number(message: Message, state: FSMContext):
    ...
    try:
        ...
        data = await state.get_data()
        text, reply_markup = inline_kb_create_complaint(next_text='Пришлите номер накладной числом без дополнительных символов', data=data)
        await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
    ...
    if invoice:
        ...
        data = await state.get_data()
        text, reply_markup = inline_kb_create_complaint(next_text='E-mail для ответа на претензию', data=data)
        await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
    else:
        await state.update_data(invoice_id=invoice_id)
        data = await state.get_data()
        text, _ = inline_kb_create_complaint(next_text='Накладная с таким номером не найдена, попробуйте ввести другой номер или нажмите "Продолжить", чтобы оставить введенный вами номер.', data=data)
        reply_markup = InlineConstructor.create_kb([['Продолжить', 'invioceid_continue']])
        await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
...
async def get_email(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    text, reply_markup = inline_kb_create_complaint(next_text='Пришлите описание ситуации', data=data)
    await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
...
async def get_situation_descriptionscription(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    text, reply_markup = inline_kb_create_complaint(next_text='Пришлите требуемую сумму в рублях', data=data)
    await data['prev_msg'].edit_text(text=text, reply_markup=reply_markup)
...
async def get_description(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    text, _ = inline_kb_create_complaint(next_text='Пришлите фото/сканы в одном сообщении (максимум 10 штук). После того как вы пришлете фото, нажмите "Продолжить"', data=data)
...
async def delete_messsage(message: Message, state: FSMContext):
