...
def inline_kb_create_invoice(next_text : str, data : dict = {}):
    keys = list(data.keys())
    text = as_line('Для того, чтобы заполнить накладную, вам нужно будет прислать боту по очереди следующие данные:')
...
def inline_kb_create_complaint(next_text : str, data : dict = {}):
    keys = list(data.keys())
    text = as_line('Для того, чтобы оформить претензию, вам нужно будет прислать боту по очереди следующие данные:')
