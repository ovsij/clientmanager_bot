...
async def set_commands(
    ...
    commands = [
        BotCommand(
            command="invoice",
            description="🟢 Добавить накладную"
        ),
        BotCommand(
            command="complaint",
            description="🔴 Подать жалобу"
        ),
        BotCommand(
            command="manager",
        ...
        for manager in managers:
            try:
                commands.append(
                    BotCommand(
                        command="myclients",
                        description="🗣 Чаты с клиентами"
                    )
                )
                commands.append(
                    BotCommand(
                        command="mycomplaints",
                        description="⚡️ Список жалоб"
                    )
                )
                await bot.set_my_commands(commands=commands, scope=BotCommandScopeChat(chat_id=manager.tg_id))
            except:
